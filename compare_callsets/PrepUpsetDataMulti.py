...
def process_base_run(jl_file, base_name, comp_name):
    """
    Consolidates Truvari results handling --pick multi one-to-many matches.
    """
    data = joblib.load(jl_file)
    # keep tpbase and fn
    # extract the base internal ID to use as the anchor
    base_df = data[data['state'].isin(['tpbase', 'fn'])].copy()
    #base_df['join_id'] = base_df['MatchId'].apply(lambda x: x if x is not None else None)
    base_df[['join_id','base_comp_id']] = base_df['MatchId'].apply(pd.Series)
    # exclude all 'fp' rows, ignoring those misleading MatchIds.
    comp_df = data[data['state'] == 'tp'][['MatchId', 'id', 'svtype']].copy()
    # In 'multi' mode, multiple TP caller variants will share the same base ID
    #comp_df['join_id'] = comp_df['MatchId'].apply(lambda x: x if x is not None else None)
    comp_df[['join_id','comp_comp_id']] = comp_df['MatchId'].apply(pd.Series)
    # merge on base ID
    merged = pd.merge(base_df, comp_df, on='join_id', how='left', suffixes=('_base', '_comp'))
    # rename columns
    ...
    # sort to prioritize tpbase
    merged['sort_rank'] = merged['state'].map({'tpbase': 0, 'fn': 1})
    merged = merged.sort_values([f'ID_{base_name}', 'sort_rank'])
    # aggregation logic. Base ID is consistant. Combine comp IDs and types delim by ;
    agg_funcs = {
        f'Type_{base_name}': 'first',
        f'ID_{comp_name}': lambda x: ';'.join([str(v) for v in x if pd.notna(v) and v != 'nan']),
        f'Type_{comp_name}': lambda x: ';'.join([str(v) for v in x if pd.notna(v) and v != 'nan']),
        'state': 'first'
    }
    df_unique = merged.groupby(f'ID_{base_name}', as_index=False).agg(agg_funcs)
    # clean empty strings
    df_unique[f'ID_{comp_name}'] = df_unique[f'ID_{comp_name}'].replace('', np.nan)
    df_unique[f'Type_{comp_name}'] = df_unique[f'Type_{comp_name}'].replace('', np.nan)
